...
---
设\( a_1 \le a_2 \le \ldots \le a_n, b_1 \le b_2 \le dots \le b_n \). 并且\( b_1+b_2+ \ldots + b_n=0$. 求证: \( \limits(\sum)_(i=1)^n a_i b_i \ge 0 \)

** 证明 **

若\(b_i\)全都等于0则结论显然成立。现在考虑\(b_n>0\)

因为\(b_1+b_2+ \dots + b_n=0 \) 且\(b_1 <=b_2<= dots <=b_n\)所以\(b_i\)可分为两部分:
\(b_1 \le b_2 \le \dots \le b_k <0\),
\( 0<b_(k+1)<=b_(k+2)<= \dots <=b_n \)

因此可记
$$ S = b_(k+1) + b_(k+2) + \dots + b_n = -(b_1+b_2+ dots + b_k) > 0 $$
